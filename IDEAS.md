...
- *Implemented*: `tags` and `task_tags` tables in DB schema. DB functions: add/remove/get tags per task, get all tags, get tasks by tag. Task object has `tags` property (comma-separated). CLI: `--add-tag TAG --add-tag-task ID`, `--remove-tag TAG --remove-tag-task ID`, `--list-tags`, `--tag TAG` filter for `--list-tasks`. Tags appear in `--list-tasks`, `--task-details`, `--export-csv`, `--export-json`. UI: Tags entry in new/edit task dialogs (comma-separated input), Tags column in main column view (sortable, ellipsized, dim style).
